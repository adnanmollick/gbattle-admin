...
export default function MatchesTab() {
  ...
  const autoCalcPrize = () => {
    ...
    if (entry===0 || players===0) { setMsg("এন্ট্রি ফি ও প্লেয়ার সংখ্যা দিন"); setTimeout(()=>setMsg(""),2500); return; }
    const total = entry * players;
    const afterProfit = total * (1 - profitPct/100);
    // round down to nearest 50
    const pool = Math.floor(afterProfit/50)*50;
    // Distribute: 50% 1st, 30% 2nd, 20% 3rd (rounded to 50)
    ...
    setMsg(`✅ মোট ৳${total} → পুল ৳${pool} (লাভ ৳${total-pool})`);
  ...
  const cancelMatch = async (m) => {
    ...
    for (const p of (m.participants||[])) {
      ...
      if (p.uid && refund>0 && !m.isFree) {
        await updateDoc(doc(db,"users",p.uid), { balance:increment(refund) });
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", backdropFilter:"blur(8px)", display:"flex", alignItems:"flex-start", justifyContent:"center", zIndex:200, overflowY:"auto", padding:"20px 0" }}>
          <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:18, padding:22, width:"94%", maxWidth:520, margin:"auto" }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <label style={{ ...label, marginBottom:0 }}>পুরস্কার তালিকা</label>
              <button onClick={autoCalcPrize} style={{ background:"linear-gradient(135deg,#7c3aed,#a78bfa)", border:"none", borderRadius:10, padding:"7px 14px", color:"#fff", fontSize:11, fontFamily:"Orbitron,sans-serif", fontWeight:700, cursor:"pointer" }}>⚡ অটো হিসাব</button>
            </div>
            {form.prizes.map((p,i)=>(

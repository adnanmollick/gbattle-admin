...
function CategoriesSection() {
  ...
  // Preload 6 default Free Fire categories (with slot + format defaults)
  const DEFAULT_CATS = [
    { name:"BR Match", emoji:"🔫", format:"Solo", maxPlayers:48 },
    { name:"Clash Squad", emoji:"⚔️", format:"Squad", maxPlayers:8 },
    { name:"CS 2V2", emoji:"🤝", format:"Duo", maxPlayers:4 },
    { name:"Lone Wolf", emoji:"🐺", format:"Solo", maxPlayers:2 },
    { name:"Lone Wolf 1v1", emoji:"⚡", format:"Solo", maxPlayers:2 },
    { name:"Free Match", emoji:"🎁", format:"Solo", maxPlayers:48 },
  ];
  const preloadDefaults = async () => {
    if (!window.confirm("৬টি ডিফল্ট ক্যাটাগরি যোগ করবেন? (যেগুলো আগে নেই)")) return;
    setSaving(true);
    try {
      const existing = cats.map(c => c.name.toLowerCase());
      let added = 0;
      for (let i=0; i<DEFAULT_CATS.length; i++) {
        const dc = DEFAULT_CATS[i];
        if (existing.includes(dc.name.toLowerCase())) continue;
        await addDoc(collection(db, "categories"), {
          name:dc.name, emoji:dc.emoji, thumbnail:"",
          format:dc.format, maxPlayers:dc.maxPlayers,
          visible:true, order:cats.length+added, createdAt:new Date().toISOString(),
        });
        added++;
      }
      setMsg(added>0?`✅ ${added}টি ক্যাটাগরি যোগ হয়েছে`:"সব ক্যাটাগরি আগে থেকেই আছে");
      setTimeout(()=>setMsg(""),2500);
    } catch(e) { setMsg("সমস্যা: "+e.message); }
    setSaving(false);
  };
  ...
  // Add/change image on existing category (image replaces emoji on display)
  const changeCatImage = async (catId, file) => {
    if (!file) return;
    try {
      const img = await compressImg(file, 500, 0.72);
      await updateDoc(doc(db, "categories", catId), { thumbnail: img });
      setMsg("✅ ছবি যোগ হয়েছে"); setTimeout(() => setMsg(""), 2000);
    } catch { setMsg("ছবি ব্যর্থ"); }
  };
  const removeCatImage = async (catId) => {
    await updateDoc(doc(db, "categories", catId), { thumbnail: "" });
    setMsg("✅ ছবি সরানো হয়েছে — এখন ইমোজি দেখাবে"); setTimeout(() => setMsg(""), 2000);
  };
  ...
  return (
    <div>
      ...
      {/* PRELOAD DEFAULTS */}
      {cats.length === 0 && (
        <div style={{ ...card, marginBottom: 16, background:"rgba(124,58,237,0.06)", border:"1px solid rgba(167,139,250,0.25)" }}>
          <div style={{ fontSize:12, color:C.text, marginBottom:4, fontWeight:600 }}>⚡ দ্রুত শুরু</div>
          <div style={{ fontSize:11, color:C.textFaint, marginBottom:12, lineHeight:1.5 }}>স্ট্যান্ডার্ড ৬টি ক্যাটাগরি (BR Match, Clash Squad, CS 2V2, Lone Wolf, ইত্যাদি) একসাথে যোগ করুন। পরে এডিট বা ডিলিট করতে পারবেন।</div>
          <button onClick={preloadDefaults} disabled={saving} style={{ ...btn("primary"), width:"100%", opacity:saving?0.7:1 }}>{saving?"যোগ হচ্ছে...":"🎮 ডিফল্ট ক্যাটাগরি যোগ করুন"}</button>
        </div>
      )}

      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 12 }}>
        <div style={{ fontFamily: "Orbitron,sans-serif", fontSize: 12, fontWeight: 700, color: C.textDim }}>ক্যাটাগরি সমূহ ({cats.length})</div>
        {cats.length > 0 && <button onClick={preloadDefaults} disabled={saving} style={{ ...btn("ghost"), padding:"5px 12px", fontSize:10 }}>+ ডিফল্ট</button>}
      </div>
      {cats.map((c) => (
        <div key={c.id} style={{ ...card, marginBottom: 10, padding: 0, overflow: "hidden", display: "flex", alignItems: "center", opacity: c.visible ? 1 : 0.5 }}>
          ...
          <div style={{ flex: 1, padding: "10px 14px", minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: c.visible ? C.green : C.textFaint }}>{c.visible ? "👁 দৃশ্যমান" : "🚫 লুকানো"}{c.thumbnail ? " · 🖼️ ছবি আছে" : ""}</div>
          </div>
          {/* IMAGE add/change */}
          <label style={{ ...btn("ghost"), margin: "0 4px", padding: "8px 10px", cursor: "pointer", fontSize: 13 }}>
            {c.thumbnail ? "🔄" : "📷"}
            <input type="file" accept="image/*" onChange={e => { changeCatImage(c.id, e.target.files[0]); e.target.value = ""; }} style={{ display: "none" }} />
          </label>
          {c.thumbnail && <button onClick={() => removeCatImage(c.id)} style={{ ...btn("ghost"), margin: "0 4px", padding: "8px 10px", fontSize: 13 }}>🖼️❌</button>}
          <button onClick={() => toggleVisible(c)} style={{ ...btn("ghost"), margin: "0 4px", padding: "8px 10px" }}>{c.visible ? "🙈" : "👁"}</button>
          <button onClick={() => removeCat(c.id)} style={{ ...btn("red"), marginRight: 10, padding: "8px 10px" }}>🗑</button>
        </div>
      ))}
    </div>
  );

...
export default function ResultsTab() {
  ...
  const publish = async () => {
    ...
    try {
      ...
      // AUTO COMMISSION → match creator admin's wallet
      if (selected.createdBy) {
        const collected = (Number(selected.entryFee)||0) * (selected.participants?.length||0);
        const prizesPaid = winners.reduce((s,w)=>s+(Number(w.prize)||0), 0);
        const profit = Math.max(0, collected - prizesPaid);
        if (profit > 0) {
          const adminSnap = await getDoc(doc(db,"users",selected.createdBy));
          if (adminSnap.exists()) {
            const commissionPct = adminSnap.data().commissionPercent ?? 20;
            const commission = Math.round(profit * commissionPct / 100);
            if (commission > 0) {
              await updateDoc(doc(db,"users",selected.createdBy), { wallet:increment(commission) });
              await addDoc(collection(db,"transactions"), {
                uid:selected.createdBy, userName:adminSnap.data().name||"Admin",
                type:"commission", amount:commission, matchTitle:selected.title,
                status:"completed", createdAt:new Date().toISOString(),
              });
              await addDoc(collection(db,"notifications"), {
                uid:selected.createdBy, type:"prize", title:"💰 কমিশন পেয়েছেন",
                body:`"${selected.title}" ম্যাচ থেকে ৳${commission} কমিশন (লাভ ৳${profit} এর ${commissionPct}%)`,
                read:false, createdAt:new Date().toISOString(),
              });
            }
          }
      ...
      <div style={{ ...card, marginBottom:16 }}>
        ...
        {selected && (
          <>
            ...
            {/* SLOT GRID overview (click slot → highlight that row) */}
            {rows.length>0 && (
              <div style={{ marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  <span style={{ fontSize:12, color:C.textDim, fontWeight:600 }}>📍 স্লট ওভারভিউ</span>
                  <span style={{ fontSize:10, color:C.textFaint }}>স্লটে ক্লিক করুন → নিচে যান</span>
                </div>
                <SlotGrid
                  participants={rows}
                  teamSize={selected.teamSize||1}
                  maxPlayers={selected.maxPlayers||48}
                  selectedSlot={highlightSlot}
                  resultMode={true}
                  onSlotClick={(slot)=>{
                    setHighlightSlot(slot);
                    const el = document.getElementById(`result-row-${slot}`);
            ...
            {rows.length===0 ? (
              ...
              return (
                <div key={i} id={`result-row-${r.slot}`} style={{ background:highlightSlot===r.slot?"rgba(167,139,250,0.12)":C.surface2, border:highlightSlot===r.slot?"1px solid rgba(167,139,250,0.4)":"1px solid transparent", borderRadius:12, padding:12, marginBottom:10, transition:"all 0.3s" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:r.players?8:0 }}>
                    <span style={{ background:"rgba(167,139,250,0.15)", border:"1px solid rgba(167,139,250,0.3)", borderRadius:7, padding:"3px 7px", fontSize:10, fontFamily:"Orbitron,sans-serif", color:C.primary, flexShrink:0 }}>#{r.slot}</span>

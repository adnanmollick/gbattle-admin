...
import { C, card, fmt, input, btn } from "../../utils/ui";
...
function BroadcastSection() {
  ...
  const send = async () => {
    if (!title.trim()||!body.trim()) { setMsg("সব তথ্য দিন"); setTimeout(()=>setMsg(""),2000); return; }
    setSending(true);
    try {
      const { collection, getDocs, addDoc } = await import("firebase/firestore");
      const snap = await getDocs(collection(db,"users"));
      for (const u of snap.docs) {
        await addDoc(collection(db,"notifications"), { uid:u.id, type:"broadcast", title:title.trim(), body:body.trim(), read:false, createdAt:new Date().toISOString() });
      }
      setMsg(`✅ ${snap.size} জনকে পাঠানো হয়েছে`); setTitle(""); setBody("");
  ...
  return (
    <div style={{ ...card, marginTop:24 }}>
      <div style={{ fontFamily:"Orbitron,sans-serif", fontSize:13, fontWeight:700, color:C.text, marginBottom:6 }}>📢 ব্রডকাস্ট নোটিফিকেশন</div>
      <div style={{ fontSize:11, color:C.textFaint, marginBottom:14 }}>সব ইউজারকে একসাথে নোটিফিকেশন পাঠান</div>
      {msg && <div style={{ background:msg.startsWith("✅")?C.greenBg:C.redBg, color:msg.startsWith("✅")?C.green:C.red, borderRadius:8, padding:"8px 12px", fontSize:12, marginBottom:12, textAlign:"center" }}>{msg}</div>}
      <input style={{ ...input, marginBottom:10 }} value={title} onChange={e=>setTitle(e.target.value)} placeholder="শিরোনাম (যেমন: নতুন টুর্নামেন্ট!)" />
      <textarea style={{ ...input, minHeight:60, resize:"vertical", marginBottom:12 }} value={body} onChange={e=>setBody(e.target.value)} placeholder="বার্তা..." />
      <button onClick={send} disabled={sending} style={{ ...btn("primary"), width:"100%", opacity:sending?0.7:1 }}>{sending?"পাঠানো হচ্ছে...":"📢 সবাইকে পাঠান"}</button>
    </div>
  );
}
...
function MonthlyAnalytics() {
  ...
  const [data, setData] = useState({ deposits:0, withdrawals:0, entryFees:0, prizes:0 });
  useEffect(() => {
    
    return onSnapshot(collection(db,"transactions"), snap => {
      let deposits=0, withdrawals=0, entryFees=0, prizes=0;
      snap.docs.forEach(d => {
        const t = d.data();
        if (!t.createdAt?.startsWith(month)) return;
        const amt = Number(t.amount)||0;
        if (t.type==="deposit" && t.status==="approved") deposits+=amt;
        else if (t.type==="withdraw" && t.status==="approved") withdrawals+=amt;
        else if (t.type==="entry") entryFees+=amt;
        else if (t.type==="prize") prizes+=amt;
      });
      setData({ deposits, withdrawals, entryFees, prizes });
    });
  }, [month]);
  const netProfit = data.entryFees - data.prizes;
  return (
    <div style={{ ...card, marginTop:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div style={{ fontFamily:"Orbitron,sans-serif", fontSize:13, fontWeight:700, color:C.text }}>📊 মাসিক রিপোর্ট</div>
      ...
      {[
        ["মোট ডিপোজিট", data.deposits, C.green],
        ["মোট উত্তোলন", data.withdrawals, C.red],
        ["এন্ট্রি ফি আয়", data.entryFees, C.blue],
        ["পুরস্কার প্রদান", data.prizes, C.amber],
      ].map(([label,val,color])=>(
        <div key={label} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:`1px solid ${C.border}` }}>
          ...
          <span style={{ fontFamily:"Orbitron,sans-serif", fontSize:13, fontWeight:700, color }}>৳{val.toLocaleString()}</span>
        </div>
      ))}
      <div style={{ display:"flex", justifyContent:"space-between", padding:"12px 0 0", marginTop:4 }}>
        <span style={{ fontSize:14, fontWeight:700, color:C.text }}>নেট লাভ</span>
        <span style={{ fontFamily:"Orbitron,sans-serif", fontSize:16, fontWeight:800, color:netProfit>=0?C.green:C.red }}>৳{netProfit.toLocaleString()}</span>
      </div>
    </div>
  );
}
